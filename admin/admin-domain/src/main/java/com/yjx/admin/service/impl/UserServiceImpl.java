...
import com.yjx.admin.mapper.RoleMapper;
...
import com.yjx.admin.transform.SystemUserStructMapper;
...
public class UserServiceImpl extends ServiceImpl<SystemUserMapper, SystemUser> implements UserService {
    ...
    public UserAuthDTO getUserByUsername(String username) {
        ...
        // 如果用户存在  查询权限并组合
        List<String> role = roleMapper.getRoleNameByUserId(user.getId());
        UserAuthDTO userAuthDTO = SystemUserStructMapper.INSTANCE.toUserAuthDTO(user);
        userAuthDTO.setRoles(role);
